 ...
 * 要点： 1:歌词的上下平移用什么实现？
 * 用Scroller实现，Scroller只是一个工具而已， 真正实现滚动效果的还是View的scrollTo方法
 *  2：歌词的水平滚动怎么实现？
...
public class LrcView extends View implements ILrcView {
	...
	/** 默认文字的字体大小 **/
	private static final float SIZE_FOR_DEFAULT_TEXT = 38;

	/** 画高亮歌词的画笔 ***/
	private Paint mPaintForHighLightLrc;
	/** 高亮歌词的默认字体大小 ***/
	private static final float DEFAULT_SIZE_FOR_HIGHT_LIGHT_LRC = 36;
	/** 高亮歌词当前的字体大小 ***/
	private float mCurSizeForHightLightLrc = DEFAULT_SIZE_FOR_HIGHT_LIGHT_LRC;
	/** 高亮歌词的默认字体颜色 **/
	private static final int DEFAULT_COLOR_FOR_HIGHT_LIGHT_LRC = Color.parseColor("#99CC00");
	/** 高亮歌词当前的字体颜色 **/
	...
	/*** 时间线的颜色 **/
	private static final int COLOR_FOR_TIME_LINE = Color.parseColor("#99CC00");
	/** 时间文字大小 **/
	private static final int SIZE_FOR_TIME = 20;
	/** 是否画时间线 **/
	private boolean mIsDrawTimeLine = false;

	/** 歌词间默认的行距 **/
	private static final float DEFAULT_PADDING = 40;
	/** 歌词当前的行距 **/
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (!canDrag) {
				if (Math.abs(event.getRawY() - firstY) > mTouchSlop 
						&& Math.abs(event.getRawY() - firstY) > Math.abs(event.getRawX() - lastX)) {
					canDrag = true;
	...
	AnimatorUpdateListener updateListener = new AnimatorUpdateListener() {
		...
		public void onAnimationUpdate(
			...
			//invalidate();
			ViewCompat.postInvalidateOnAnimation(LrcView.this);			
		}
	};
